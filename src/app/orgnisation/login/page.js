...
const LoginPage = () => {
    ...
    const login = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await fetch('/api/auth/orgnisation/login', {
                method: 'POST',
            ...
            if (data.success) {
                setUser({ id: data.user.id, name: data.user.name, logo: data.user.logo, userType: 'org' })
            ...
            <form onSubmit={login} className='w-11/12 md:w-4/5 md:max-w-md p-4 pt-5 bg-white shadow-xlrounded-md flex flex-col gap-2 border border-text rounded-md'>
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <label className='flex gap-1 flex-wrap sm:justify-start justify-center'>
                        ...
                        <Link href='/orgnisation/signup' className='text-blue-700 hover:underline'>create account</Link>
                    </label>
                </div>
            </form>
        </section>
